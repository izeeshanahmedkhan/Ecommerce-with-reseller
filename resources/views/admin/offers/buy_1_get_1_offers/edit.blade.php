...
@section('content')
    <input type="hidden" value="{{$activePage = 'offerIndex', $title = 'Edit Buy One Get One Free - Nafia Garments'}}">
    <div class="main-content">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="card-title mb-3">Edit Buy One Get One Free</div>
                        <form class="forms-sample" method="POST" action="{{ route('buy_1_get_1_offer.update',$offer) }}">
                            @csrf()
                            @method('PUT')
                            <div class="row">
                                    ...
                                    <select class="form-control js-example-basic-single @error('product_id') is-invalid @enderror" id="product" name="product_id">
                                        ...
                                        @foreach($products as $product)
                                            @if($offer->product_id == $product->id)
                                                <option {{ $offer->product_id == $product->id ? 'selected':'' }} value="{{ $product->id }}"> {{ $product->name }} </option>
                                            @endif
                                ...
                                <div class="col-md-6 form-group mb-3">
                                    ...
                                    <select class="form-control @error('size_id') is-invalid @enderror" id="size" name="size_id">
                                        @if(!is_null($offer->size_id))
                                            <option value="{{ $offer->size_id }}"> {{ \App\Models\Size::where('id',$offer->size_id)->first()->sizeName }} </option>
                                        @endif
                                    </select>
                                    @error('size_id')
                                    <span class="invalid-feedback" role="alert">
