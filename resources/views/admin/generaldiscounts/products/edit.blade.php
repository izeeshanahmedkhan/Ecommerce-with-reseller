...
@section('content')
    <input type="hidden" value="{{$activePage = 'generaldiscountIndex', $title = 'Edit Product Discount - Nafia Garments'}}">
    <div class="main-content">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="card-title mb-3">Edit Product Discount</div>
                        <form class="forms-sample" method="POST" action="{{ route('product_discount.update',$generaldiscount) }}">
                            @csrf()
                            @method('PUT')
                            <div class="row">
                                ...
                                <div class="col-md-6 form-group mb-3">
                                    ...
                                    <select class="form-control js-example-basic-single @error('product_id') is-invalid @enderror" id="product" name="product_id">
                                        ...
                                        @foreach($products as $product)
                                            @if($generaldiscount->product_id == $product->id)
                                                <option {{ $generaldiscount->product_id == $product->id ? 'selected':'' }} value="{{ $product->id }}"> {{ $product->name }} </option>
                                            @endif
