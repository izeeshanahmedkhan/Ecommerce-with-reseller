...
@section('content')
    <input type="hidden" value="{{$activePage = 'generaldiscountIndex', $title = 'Edit Category Discount - Nafia Garments'}}">
    <div class="main-content">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="card-title mb-3">Edit Category Discount</div>
                        <form class="forms-sample" method="POST" action="{{ route('category_discount.update',$generaldiscount) }}">
                            @csrf()
                            @method('PUT')
                            <div class="row">
                                ...
                                <div class="col-md-6 form-group mb-3">
                                    ...
                                    <select class="form-control js-example-basic-single @error('category_id') is-invalid @enderror" id="category" name="category_id">
                                        ...
                                        @foreach($categories as $category)
                                            <option {{ $generaldiscount->category_id == $category->id ? 'selected':'' }} value="{{ $category->id }}"> {{ $category->title }} </option>
                                        @endforeach
                                    </select>
                                    @error('category_id')
